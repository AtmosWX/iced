...
pub fn window_attributes(
    ...
    attributes = attributes
        ...
        .with_fullscreen(
            settings
                .fullscreen
                .then_some(winit::monitor::Fullscreen::Borderless(None)),
        )
...
pub fn window_event(
    ...
    match event {
        ...
        WindowEvent::DragEntered { paths, .. } if !paths.is_empty() => {
            Some(Event::Window(window::Event::FileHovered(
                paths.first().unwrap().clone(),
            )))
        }
        WindowEvent::DragDropped { paths, .. } if !paths.is_empty() => {
    ...
    match position {
        ...
        window::Position::SpecificWith(to_position) => {
            if let Some(monitor) = monitor {
                let start = monitor.position().expect("No monitor position");
                let video_mode =
                    monitor.current_video_mode().expect("No video mode");

                let resolution: winit::dpi::LogicalSize<f32> =
                    video_mode.size().to_logical(monitor.scale_factor());
...
pub fn fullscreen(
    ...
    match mode {
        ...
        window::Mode::Fullscreen => {
            Some(winit::monitor::Fullscreen::Borderless(monitor))
        }
    }
}
...
pub fn mouse_interaction(
    ...
    match interaction {
        Interaction::None | Interaction::Idle => {
            winit::window::Cursor::Icon(CursorIcon::Default)
        }
        Interaction::Pointer => {
            winit::window::Cursor::Icon(CursorIcon::Pointer)
        }
        Interaction::Working => {
            winit::window::Cursor::Icon(CursorIcon::Progress)
        }
        Interaction::Grab => winit::window::Cursor::Icon(CursorIcon::Grab),
        Interaction::Grabbing => {
            winit::window::Cursor::Icon(CursorIcon::Grabbing)
        }
        Interaction::Crosshair => {
            winit::window::Cursor::Icon(CursorIcon::Crosshair)
        }
        Interaction::Text => winit::window::Cursor::Icon(CursorIcon::Text),
        Interaction::ResizingHorizontally => {
            winit::window::Cursor::Icon(CursorIcon::EwResize)
        }
        Interaction::ResizingVertically => {
            winit::window::Cursor::Icon(CursorIcon::NsResize)
        }
        Interaction::ResizingDiagonallyUp => {
            winit::window::Cursor::Icon(CursorIcon::NeswResize)
        }
        Interaction::ResizingDiagonallyDown => {
            winit::window::Cursor::Icon(CursorIcon::NwseResize)
        }
        Interaction::NotAllowed => {
            winit::window::Cursor::Icon(CursorIcon::NotAllowed)
        }
        Interaction::ZoomIn => winit::window::Cursor::Icon(CursorIcon::ZoomIn),
        Interaction::ZoomOut => {
            winit::window::Cursor::Icon(CursorIcon::ZoomOut)
        }
        Interaction::Cell => winit::window::Cursor::Icon(CursorIcon::Cell),
        Interaction::Move => winit::window::Cursor::Icon(CursorIcon::Move),
        Interaction::Copy => winit::window::Cursor::Icon(CursorIcon::Copy),
        Interaction::Help => winit::window::Cursor::Icon(CursorIcon::Help),
    }
}

/// Converts a `ElementState` and `ButtonSource` from [`winit`] to an [`iced`] mouse event.
///
/// [`winit`]: https://github.com/rust-windowing/winit
...
pub fn pointer_button(
    ...
    match button {
        winit::event::ButtonSource::Mouse(mouse_button) => {
            ...
            match state {
                winit::event::ElementState::Pressed => {
                    Some(Event::Mouse(mouse::Event::ButtonPressed(button)))
                }
                winit::event::ElementState::Released => {
                    Some(Event::Mouse(mouse::Event::ButtonReleased(button)))
                }
            }
        }
        winit::event::ButtonSource::Touch { .. }
        | winit::event::ButtonSource::Unknown(_) => None,
    }
}
